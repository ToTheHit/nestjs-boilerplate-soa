...
const sendDelayedEventNX = async (id, event, content, time, queue) =>
  sender('setEvent', {}, queueTimer, {
    id: `${event}_${id}`,
    time,
    routingKey: queue,
...
const sendDelayedEvent = async (id, event, content, time, queue) =>
  sender('setEvent', {}, queueTimer, {
    id: `${event}_${id}`,
    time,
    routingKey: queue,
...
const clearDelayedEvent = async (ids, event) =>
  Promise.all(
